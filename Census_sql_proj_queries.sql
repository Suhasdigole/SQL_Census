...
select * from project..data1 where state in ('Maharashtra','Goa')

-- population of India

select sum(population) as Population from project..data2

-- avg growth of population state wise
...
-- avg literacy rate >70
 
select state,round(avg(literacy),0) Avg_literacy_rate from project..data1 
group by state having round(avg(literacy),0)>70 order by Avg_literacy_rate desc ;


-- top 3 state showing highest growth ratio using limit 3

select top 3 state,avg(growth)*100 avg_growth from project..data1 group by state order by avg_growth desc;


--bottom 3 state showing lowest sex ratio using top 3
...
-- state staring with A ending with M
